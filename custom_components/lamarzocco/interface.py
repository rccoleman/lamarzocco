...
class LMInterface:
    ...
    def __init__(self, config):
        ...
        if self.model_name in LM_CLOUD_MODELS:
            self.lm_cloud = LMCloud.create(config, config["ip"], config["port"])
        else:
    ...
    async def connect(self):
        if self.model_name in LM_CLOUD_MODELS:
            ...
            self.lm_direct.connect()


    async def request_status(self):
        if self.model_name in LM_CLOUD_MODELS:
            self.lm_cloud.get_status()
        else:
            self.lm_direct.request_status()
    ...
    async def set_auto_on_off_enable(self, day_of_week, enable):
        if self.model_name in LM_CLOUD_MODELS:
            ...
            self.lm_cloud.configure_schedule(True, schedule)
        else:
            self.lm_direct.set_auto_on_off_enable(day_of_week, enable)


    async def set_auto_on_off_times(self, day_of_week, hour_on, minute_on, hour_off, minute_off):
        if self.model_name in LM_CLOUD_MODELS:
            self.lm_cloud.set_auto_on_off(day_of_week, hour_on, minute_on, hour_off, minute_off)
        else:
            self.lm_direct.set_auto_on_off_times(day_of_week, hour_on, minute_on, hour_off, minute_off)
    ...
    async def set_dose(self, key, pulses):
        if self.model_name in LM_CLOUD_MODELS:
            ...
            self.lm_direct.set_dose(key, pulses)

    async def set_prebrew_times(self, key, seconds_on, seconds_off):
        if self.model_name in LM_CLOUD_MODELS:
            self.lm_cloud.configure_prebrew(prebrewOnTime=seconds_on, prebrewOffTime=seconds_off)
        else:
            self.lm_direct.set_prebrew_times(key, seconds_on, seconds_off)

    async def set_preinfusion_time(self, key, seconds):
        if self.model_name in LM_CLOUD_MODELS:
            pass
        else:
            self.lm_direct.set_preinfusion_time(key, seconds)